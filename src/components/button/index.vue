<template>
  <button @click="change" :disabled="disabled || loading " class="ele-button" :style="[minWidthCss]" :class="[theme, isRound, isBorder, isSize, blockCss]">
    <span>
...
export default {
  ...
  computed: {
    theme () {
      return this.type ? `ele-button-${this.type}` : ''
    },
    isRound () {
    ...
    isSize () {
      return this.size ? `ele-button-${this.size}` : ''
    },
    minWidthCss () {
    ...
    blockCss () {
      return this.block ? 'ele-button-block' : ''
    }
  },
  methods: {
    change () {
